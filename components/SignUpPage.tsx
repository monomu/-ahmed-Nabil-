...
interface SignUpPageProps {
    onAttemptSignUp: (name: string, email: string, pass: string) => Promise<string | void>;
    onSetView: (view: View) => void;
}

const SignUpPage: React.FC<SignUpPageProps> = ({ onAttemptSignUp, onSetView }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        const result = await onAttemptSignUp(name, email, password);
        setLoading(false);
        if (result) {
            setError(result);
        }
        ...
        <div className="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                ...
                <p className="mt-2 text-center text-sm text-gray-600">
                    أو{' '}
                    <button onClick={() => onSetView('login')} className="font-medium text-teal-600 hover:text-teal-500">
